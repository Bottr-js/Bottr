...
function Bot(config) {
  ...
  this.contextStore = Redis.createClient();
}
...
Bot.prototype.start = function() {
  ...
  console.log("Connecting to Context Store");

  var connectedToContextStoreHandler = function() {
  ...
  this.contextStore.on('connect', connectedToContextStoreHandler.bind(this));
};

Bot.prototype.didRecieveMessage = function(message, user, callback) {
  var context = this.contextForUser(user, function() {
    var request = {
      context: context,
      message: message,
      user: user
