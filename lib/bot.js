...
function Bot(name) {
  ...
  this.eventEmitter.addListener('message_received', function(message, session, next) {
    ...
    if (context._conversation) {
      var converation = this.conversations[context._conversation]
...
Bot.prototype.startConversation = function(session, callback) {

  var id = uuid.v4()
  var context = session.getUserContext()
  var conversation = new Conversation(session)
...
Conversation.prototype.finishConversation = function(conversation) {
